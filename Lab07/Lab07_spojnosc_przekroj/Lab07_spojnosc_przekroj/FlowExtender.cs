...
namespace ASD
{
    public static class FlowExtender
    {
        ...
        public static double MinCut(this Graph<double> undirectedGraph, int s, int t, out Edge<double>[] minCut)
        {
            ...
            Graph<double> residualGraph = new Graph<double>(undirectedGraph.VertexCount);
            
            // przejdz po wszystkich wiercholkach 
            for(int v = 0; v < undirectedGraph.VertexCount; v++)
            {
                ...
                foreach (Edge<double> e in undirectedGraph.OutEdges(v))
                {
                    if (flow.HasEdge(e.From, e.To))
                    {
                        if(flow.GetEdgeWeight(e.From, e.To) < e.Weight)
                        {
                            residualGraph.AddEdge(e.From, e.To, e.Weight - flow.GetEdgeWeight(e.From, e.To));
                        
                        }
                        residualGraph.AddEdge(e.To, e.From, flow.GetEdgeWeight(e.From, e.To));
            ...
            bool[] visited = new bool[undirectedGraph.VertexCount];
            visited[s] = true;
